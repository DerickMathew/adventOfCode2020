...
def removeParenthesisRecursively(equation):
	...
	newEquationBegin = equation[:startingIndex]
	newEquationEnd = equation[(startingIndex + len(matchedEquation)):]
	evaluatedExpression = getflattenedEquation(matchedEquation[1:-1])
	newEquation = newEquationBegin + evaluatedExpression + newEquationEnd
...
def getflattenedEquation(equation):
	...
	return str(evaluateFlatEquation(equation))
...
def evaluateFlatEquation(flatEquation):
	...
	while len(operators) > 0:
		(operators, operands) = executeOperator(operators, operands, 0)
